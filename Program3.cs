...
class Program
{
    ...
    {
        Football[] _2massives = new Football[top1.Length + top2.Length];
        int index1 = 0;
        int index2 = 0;
        int index2massives = 0;
        while (index1 < top1.Length && index2 < top2.Length)
        {
            if (top1[index1].Score >= top2[index2].Score)
            {
                _2massives[index2massives] = top1[index1];
                index1++;
            }
            else
            {
                _2massives[index2massives] = top2[index2];
                index2++;
            }
            index2massives++;
        }
        while (index1 < top1.Length)
        {
            _2massives[index2massives] = top1[index1];
            index1++;
            index2massives++;
        }
        while (index2 < top2.Length)
        {
            _2massives[index2massives] = top2[index2];
            index2++;
            index2massives++;
        }
        return TopTims(_2massives, 6);
    }
}