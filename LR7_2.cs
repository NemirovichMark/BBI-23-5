namespace ConsoleApp1
{
    class Program
    {
        abstract class SportHuman
        {
            ...
            public virtual string Gender { get; } = "Спортсмен";
            ...
            public void Show()
            {
                Console.WriteLine("{0}: {1} {2, 10} - {3:f1} секунд", Team, Gender, Name, Time);
            }
        }

        class SportMan : SportHuman
        {
            public override string Gender { get; } = "Мужчина";
            public SportMan(string name, double time) : base(name, time, 0) { }
        }

        class SportWoman : SportHuman
        {
            public override string Gender { get; } = "Женщина";
