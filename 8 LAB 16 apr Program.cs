...
class TaskProcessor_ : TaskProcessor
{
...
abstract class Task
{
    ...
    public Task(string text, TaskProcessor processor)
    {
...
class Task_1 : Task
{
    public Task_1(string text, TaskProcessor processor) : base(text, processor) { }
    ...
    {
        processedText = ProcessText(processedText); // Обновляем processedText, вызвав ProcessText()
        return processedText;
    }
}
class Task_2 : Task
{
    ...
    public Task_2(string text, TaskProcessor processor) : base(text, processor)
    {
...
{
    private List<KeyValuePair<string, char>> infoposleds;
    public Task_3(string text, TaskProcessor processor, List<KeyValuePair<string, char>> infoposleds) : base(text, processor)
    {
    ...
    protected override string ProcessText(string text)
    {
        return DecodeText(text, infoposleds.ToDictionary(k => k.Key, v => v.Value.ToString()));
    }
    public override string ToString()
    {
        return processedText;
...
class Task_4 : Task
{
    ...
    public Task_4(string text, TaskProcessor_ processor, string[] codes) : base(text, processor)
    {
        this.codes = codes;
        encodedText = EncodeText(text, codes);
        processedText = DecodeText(encodedText, codes.ToDictionary(k => k, v => ((char)('v' + Array.IndexOf(codes, v))).ToString()));
    }
    protected override string ProcessText(string text)
    {
