...
class Program
{
    ...
    {
        students[] list = { new maths("Сидоров", 4, 3), new informatics("Арбузов", 2, 20), new maths("Зайцев", 2, 7), new informatics("Иванов", 2, 10) };
        ...
        Console.WriteLine("Неуспевающие по математике");
        for (int i = 0; i < bad_students_maths.Length; i++)
        {
            Console.WriteLine($"ФИО: {bad_students_maths[i].fio}, Оценка: {bad_students_maths[i].mark}, Пропуски: {bad_students_maths[i].misses}");
        }
        Console.WriteLine("Неуспевающие по информатике");
        for (int i = 0; i < bad_students_informatics.Length; i++)
        {
            Console.WriteLine($"ФИО: {bad_students_informatics[i].fio}, Оценка: {bad_students_informatics[i].mark}, Пропуски: {bad_students_informatics[i].misses}");
        }
