...
internal class Program
{
    ...
    class Task1 : Task
    {
        public Task1(string text) : base(text)
        {
            ...
            // Добавляем результаты в output
            foreach (var x in top)
            {
                output += $"{x.Key}: {(float)x.Value * 100 / totalRusLetters:F2}%\n";
            }
        }
    }
        ...
        public Task5(string text) : base(text)
        {
            ...
            foreach (string word in words)
            {
                if (word.Length > 0)
                {
                    ...
                    if (!nums.Contains(sym) && char.IsLetter(sym))
                    {
                        if (top.ContainsKey(sym))
                        {
                            top[sym]++;
            ...
            int all = 0;
            foreach (int x in sorted.Values)
            {
                all += x;
            }
