abstract class Jump
{
    ...
    public double Rez { get { return Get_Rez(1); } }
}

class Jump120 : Jump
{
    protected string disciplineName;
    public Jump120(string disciplineName, string famile, double[] estimate, int distance) : base(famile, estimate, distance)
    {
        this.disciplineName = "120m jump";
        rez = 120;
    }
    ...
    {
        Console.WriteLine(famile + " " + Get_Rez(120));
    }
...
class Jump180 : Jump
{
    ...
    protected string disciplineName;
    public Jump180(string disciplineName, string famile, double[] estimate, int distance) : base(famile, estimate, distance)
    {

        this.disciplineName = "180m jump";
        rez = 180;
    ...
    public override void GetInfo()
    {
        Console.WriteLine(famile + " " + Get_Rez(180));
    }
...
class Program
{
    static void Main(string[] args)
    {
        ...
        {
            double amax = jump120[i].Rez;
            int imax = i;
            for (int j = i + 1; j < jump120.Length; j++)
            {
                if (amax < jump120[j].Rez)
                {
                    amax = jump120[j].Rez;
                    imax = j;
                }
        ...
        {
            double amax = jump180[i].Rez;
            int imax = i;
            for (int j = i + 1; j < jump180.Length; j++)
            {
                if (amax < jump180[j].Rez)
                {
                    amax = jump180[j].Rez;
                    imax = j;
                }
